...
const addInterview = asyncHandler(async (req, res) => {
  ...
  usersInvited.forEach((userMail) => {
    emailSender({
      email: userMail,
      subject: `Scaler Interview @ ${moment(startTime).format(
        "DD-MM-YYYY"
      )}, ${moment(startTime).format("hh:mm A")} - ${moment(endTime).format(
        "hh:mm A"
      )}`,
      body: `Hi,
      We are from Scaler, Your Interview is scheduled on ${moment(startTime).format(
        "DD-MM-YYYY"
      )}, 
...
const getUpcomingInterviews = asyncHandler(async (req, res) => {
  let interviews = await Interview.find({ startTime: { $gte: new Date() } }).populate('usersInvited')
    .lean()
    .exec();
